...
#define NUMBER_OF_X_THREADS 1
#define NUMBER_OF_THREADS (NUMBER_OF_C_THREADS+NUMBER_OF_P_THREADS+NUMBER_OF_X_THREADS)
...
int main(int argc, char **argv)
{
    ...
    if ( (semC = sem_open("/my_semaphore3", O_CREAT, 0777, 0)) == SEM_FAILED ) {
    ...
    for (int i = 0; i < NUMBER_OF_X_THREADS; ++i) {
        err = pthread_create(&tid[thread_count], NULL, thread_x, (void *) thread_texts[thread_count]);
        if (err != 0)
            std::cerr << "Can't create thread. Error: " << strerror(err) << std::endl;
        else
            ++thread_count;
